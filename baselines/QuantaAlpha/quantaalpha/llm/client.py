        ...
        *,
        ...
        if self.use_llama2:
            ...
            if self.chat_stream:
                ...
                usage_log_path = os.environ.get("LLM_USAGE_LOG_PATH", "")
                if usage_log_path and usage is not None:
                    try:
                        usage_record = {
                            "tag": tag,
                            "model": model,
                        ...
                        with usage_path.open("a") as usage_file:
                            usage_file.write(json.dumps(usage_record) + "\n")
                    except Exception as exc:
                        logger.warning(f"Failed to write LLM usage log: {exc}")
                if LLM_SETTINGS.log_llm_chat_content:
                    display_resp = resp[:200] + f"... [{len(resp)} chars]" if len(resp) > 200 else resp
                    logger.info(f"{LogColors.CYAN}Response:{display_resp}{LogColors.END}", tag="llm_messages")
                    logger.info(
